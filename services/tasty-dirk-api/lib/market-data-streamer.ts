...
import WebSocket from '../node_modules/isomorphic-ws'
...
const { v4 } = require('../node_modules/uuid/dist')
const uuidv4: any = v4
...
export default class MarketDataStreamer {
  ...
  disconnect() {
    ...
    if (this.webSocket) this.webSocket.onopen = null
    if (this.webSocket) this.webSocket.onerror = null
    if (this.webSocket) this.webSocket.onmessage = null
    if (this.webSocket) this.webSocket.onclose = null
  ...
  private queueSubscription(symbol: string, options: SubscriptionOptions) {
    ...
    if (queue) queue.push({ symbol, subscriptionTypes, subscriptionArgs })
  }
  ...
  private sendQueuedSubscriptions(channelId: number) {
    ...
    this.subscriptionsQueue.set(channelId, [])
    if (queuedSubscriptions) queuedSubscriptions.forEach(subscription => {
  ...
  private sendMessage(json: object) {
    ...
    if (this.webSocket) this.webSocket.send(JSON.stringify(json))
  }
}